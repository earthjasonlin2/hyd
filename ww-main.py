...
# Base URLs
base_url = "https://api.hakush.in/ww/data/"
...
chlr.setLevel('INFO')
fhlrd = logging.FileHandler("./ww-app-debug.log", encoding='utf-8')
fhlrd.setFormatter(formatter)
fhlri = logging.FileHandler("./ww-app-info.log", encoding='utf-8')
...
# Download new.json
download_file('https://api.hakush.in/ww/new.json', os.path.join(save_dir, "../new.json"))
...
echo_data = load_json(os.path.join(save_dir, "echo.json"))
mosnter_data = load_json(os.path.join(save_dir, "monster.json"))
...
echo_ids = echo_data.keys()
monster_ids = mosnter_data.keys()
tower_ids = tower_data.keys()
...
download_localized_files("echo", echo_ids)
